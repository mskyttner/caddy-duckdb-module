...
func TestOpenAPIHandler_Spec_Info(t *testing.T) {
	...
	if info["version"] != "1.1.0" {
		t.Errorf("Expected version '1.1.0', got '%v'", info["version"])
	}
...
func TestOpenAPIHandler_Spec_Tags(t *testing.T) {
	...
	// Verify tag names
	expectedTags := map[string]bool{"CRUD": false, "Query": false, "Macro": false, "View": false, "Schema": false, "OpenAPI": false}
